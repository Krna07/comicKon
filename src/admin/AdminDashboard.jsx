...
import {
  ...
  TrendingUp, ArrowUpRight, Star, Crown, SortAsc
} from 'lucide-react';
...
function PanelCard({ panel, index, localPage, onLocalPageChange, onEdit, onDelete }) {
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex group hover:border-orange-200 transition-all duration-200"
    >
      {/* Position input */}
      <div className="flex flex-col items-center justify-center bg-gray-50 border-r border-gray-100 px-3 py-3 gap-1 min-w-[58px]">
        <span className="text-gray-400 text-[9px] font-bold uppercase tracking-wider">Order</span>
        <input
          type="number"
          min="1"
          value={localPage}
          ...
          title="Enter position number — lower = appears first"
        />
      </div>
      ...
      <div className="w-20 h-20 flex-shrink-0 bg-gray-50 overflow-hidden relative">
        ...
        {isFirst && (
          <div className="absolute top-1 left-1 bg-yellow-400 text-white rounded-full p-0.5 shadow" title="First page">
            <Crown size={9} />
          </div>
        )}
      </div>
      ...
      <div className="flex-1 px-4 py-3 min-w-0">
        <div className="flex items-center gap-1.5 mb-1.5 flex-wrap">
          ...
          <span className="text-gray-300 text-xs">pg {panel.pageNumber}</span>
        </div>
...
function PanelModal({ panel, onClose, onSave, loading }) {
  ...
  const sizeOptions = [
    { val: 'wide',  label: 'Full Width', icon: '▬', color: 'blue'   },
    { val: 'half',  label: 'Half',       icon: '▪▪', color: 'purple' },
    { val: 'third', label: 'Third',      icon: '▫▫▫', color: 'green' },
  ];
        ...
        <div className="p-6 flex flex-col gap-5">
          ...
          <div>
            ...
            <div
              ...
              className={`relative border-2 border-dashed rounded-2xl cursor-pointer transition-all overflow-hidden
                ...
                : preview   ? 'border-orange-200 bg-orange-50/30'
              ...
              {preview ? (
                <div className="relative">
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 hover:opacity-100 transition-opacity flex items-center justify-center">
                    <span className="flex items-center gap-2 text-white bg-black/50 px-4 py-2 rounded-full text-sm font-medium">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              />
              <p className="text-gray-400 text-xs mt-1">Lower number = appears first</p>
            </div>
            <div>
          ...
          <div>
            <label className="text-gray-500 text-xs font-semibold uppercase tracking-wide mb-2 block flex items-center gap-1.5">
              <FileText size={11} /> Caption (Hindi) — optional
            </label>
            <textarea
              value={caption}
...
export default function AdminDashboard() {
  ...
  const [panels,   setPanels]   = useState([]);    // sorted by pageNumber
  const [analytics,setAnalytics]= useState(null);
  const [loading,  setLoading]  = useState(true);
  const [saving,   setSaving]   = useState(false);

  // localPageNumbers: { [panelNumber]: string } — what the user typed in each input
  ...
  async function loadComic() {
    try {
      ...
      setComic(res.data);
      setPanels(sorted);
      // Initialise local inputs from DB values
      const init = {};
  ...
  // Called when user changes a number input
  function handleLocalPageChange(panelNumber, value) {
    setLocalPages(prev => ({ ...prev, [panelNumber]: value }));
    setOrderDirty(true);
  }

  // Apply order: sort panels by the numbers the user entered, then save to backend
  async function applyOrder() {
    // Validate — all values must be positive integers
    const entries = panels.map(p => ({
      panelNumber: p.panelNumber,
      pageNumber: parseInt(localPages[p.panelNumber]) || p.pageNumber,
    }));

    const hasDupes = entries.length !== new Set(entries.map(e => e.pageNumber)).size;
    if (hasDupes) {
      toastError('हर पेज का नंबर अलग होना चाहिए');
      return;
    }

    setApplyingSave(true);
    try {
      await adminReorderPanels(entries);
      success('क्रम सेव हो गया ✓');
      await loadComic();   // reload to get DB-sorted order
    } catch (err) {
      toastError(err.response?.data?.message || 'Order save failed');
    } finally {
  ...
  // Panels displayed in order of their localPage inputs (preview before saving)
      ...
      <div className="flex min-h-screen">
        ...
        <aside className="hidden md:flex w-64 flex-shrink-0 flex-col bg-white border-r border-gray-100 shadow-sm">
          ...
          <nav className="flex-1 px-4 py-6 flex flex-col gap-1">
            ...
            {[
              { icon: LayoutGrid, label: 'All Pages',     active: true,  fn: undefined },
              { icon: TrendingUp, label: 'Analytics',     active: false, fn: undefined },
              { icon: Edit3,      label: 'Comic Details', active: false, fn: () => setShowMetaModal(true) },
            ].map(item => (
        ...
        <main className="flex-1 flex flex-col min-w-0">
          ...
          <header className="bg-white/80 backdrop-blur-sm border-b border-gray-100 px-6 py-4 flex items-center justify-between sticky top-0 z-30">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={logout}
                className="md:hidden p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all">
                <LogOut size={15} />
              </button>
            </div>
          </header>
          ...
          <div className="flex-1 px-4 sm:px-6 py-6 flex flex-col gap-6 max-w-4xl">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <h3 className="hindi-text text-gray-800 font-bold text-lg leading-tight truncate">{comic.title}</h3>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                {/* Apply Order button — only shown when user has made changes */}
                {orderDirty && (
                  <motion.button
                    ...
                    className="flex items-center gap-1.5 bg-orange-500 hover:bg-orange-400 disabled:opacity-50 text-white font-bold text-xs px-4 py-2 rounded-xl transition-all shadow-md shadow-orange-100"
                  >
                    {applyingSave
                      ? <><div className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Saving...</>
                      : <><SortAsc size={13} /> Apply Order</>}
                  </motion.button>
                )}
              </div>
              ...
              {panels.length > 1 && (
                <div className="flex items-start gap-2.5 bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 mb-3">
                  <SortAsc size={15} className="text-blue-500 flex-shrink-0 mt-0.5" />
                  <p className="text-blue-700 text-xs leading-relaxed">
                    <strong>Order pages:</strong> Change the number in the <span className="bg-white border border-blue-200 px-1 rounded font-mono">Order</span> box on each card — lower number appears first. Click <strong>Apply Order</strong> to save.
                  </p>
                </div>
              )}
        ...
        {deleteConfirm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div
              ...
              className="bg-white rounded-3xl p-7 max-w-xs w-full text-center shadow-2xl border border-gray-100"
              onClick={e => e.stopPropagation()}>
